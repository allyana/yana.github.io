...
<?php
	...
	$date = $_POST['date'];
	$typeConfession = $_POST['typeConfession'];
	$subject = $_POST['subject'];
	$confession = $_POST['confession'];
	if(!empty($typeConfession)){
		try{
			$confessData = $con->prepare("INSERT INTO confession
										  (studentID, date, typeConfession, subject, confession) 
										  VALUES (:studentID, :date, :typeConfession, :subject, :confession)");
					$confessData->execute(array(':studentID'=>$confessStudent, ':date'=>$date, 
												':typeConfession'=>$typeConfession, ':subject'=>$subject, 
												':confession'=>$confession));
					header('Location:eConfession.php');
				}catch(PDOException $ex){
					echo $ex->getMessage();
				}
			}else{
				echo "Choose Type of Confession";
			}
		}
?>
...
<h2 style="font-size: 36px; font-weight: bold;"><center>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Make Your Confession</h2></center><br>
<tr>
 <td valign="top">
...
<tr>
 <td valign="top">
  <label for="typeConfession">Type of Confession</label>
 </td>
 <td valign="top">
  <select name="typeConfession">
...
<tr>
 ...
 <td valign="top">
  <textarea  name="confession" maxlength="1000" cols="30" rows="6"></textarea>
 </td>
 
</tr>
<tr>
 <td colspan="2" style="text-align:center">
  <button align="center" type="submit" name="submit" class="button"> Submit</button>
 </td>
</tr>
</table>
</form>
