...
<?php
	...
	require('db_config/PDO_connect.php');
			$reportDass = $con->prepare("select s.studentID, s.faculty, d.depressionScore, d.anxietyScore, d.stressScore
										 from student s join dassresult d 
										 on s.studentID = d.studentID");
			$reportDass->execute();
			$result = $reportDass->fetchAll();
			foreach($result as $row){
?>
		<tr>
			<td><?=$row['studentID'];?></td>
			<td><?=$row['faculty'];?></td>
			<td><?=$row['depressionScore'];?></td>
