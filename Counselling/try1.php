...
{	
			$adminID=$_GET['update_id'];
			$adminData_edit=$con->prepare('SELECT * FROM admin WHERE adminID=:adminID');
            $adminData_edit->execute(array(':adminID'=>$adminID));
            $edit_row=$adminData_edit->fetch(PDO::FETCH_ASSOC);
            extract($edit_row);
        }else
        ...
        if(isset($_POST['updateAdmin']))
 
            {
                ...
                $adminData=$con->prepare('UPDATE admin SET password = :password, 
																 aEmail = :aEmail,
																 aPhoneNum = :aPhoneNum WHERE adminID=:adminID');
                $adminData->bindParam(':adminID', $adminID);
				//$staffData->bindParam(':password', $password);
				$adminData->bindParam(':password', $password);
				$adminData->bindParam(':aEmail', $aEmail);
				$adminData->bindParam(':aPhoneNum', $aPhoneNum);
				//$staffData->bindParam(':officeRoom', $officeRoom);
				
                if($adminData->execute())
                {
                    ?>
...
<div>
  <h2 align="center">Edit Administrator Information</h2>
	<form method="POST" action="adminIndex.php">
		...
		<div class="input-group">
			<label>Email</label>
			<input type="text" name="aEmail" value="<?=$aEmail;?>">
		</div>
		<div class="input-group">
