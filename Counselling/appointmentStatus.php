...
<header id="portfolio" style="background-color:#95a5a6">
  ...
  <h2 align="center"> Appointment Status</h2><br><br>
...
<?php
	require('db_config/PDO_connect.php');
			$statData = $con->prepare("select a.date, a.time, p.statusBook, p.description 
									   from appointment a join approve p 
									   on a.appointmentID=p.appointmentID 
									   where p.studentID='$userProfile'");
			$statData->execute();
			$result = $statData->fetchAll();
