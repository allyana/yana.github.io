...
if(isset($_REQUEST["loginStud"]))
{
	  $studentID=$_REQUEST["studentID"];
	  $password=$_REQUEST["password"];
	  $query=mysql_query("select sName, picProfile from student where studentID='$studentID' && password='$password'");
...
<h2><center> Student Login Form</h2></center>
		<h3><center><form class="myform" action="" method="post">
		<div class="message"><?php //if($message!="") { echo $message; } ?></div>
		
			<label> Username: </label><br>
			<input id="studentID" name="studentID" type="text" class="inputvalues" placeholder="Type your username"/><br>
			<label> Password: </label><br>
			<input id="password" name="password" type="password" class="inputvalues" placeholder=""/><br>
			<input name="loginStud" type="submit" id="login_btn" value="Login"/><br><br>
		</form></center></h3>
	</div>
</body>
</html> 