...
?>
	...
	require("db_config/PDO_connect.php");
		if(isset($_POST['approve'])){
			...
			if(!empty($statusBook)){
				try{
				$stmt = $con->prepare("SELECT appointmentID, studentID FROM appointment");  
				$stmt->execute();
			...
			$stmt = $con->prepare("INSERT INTO approve(appointmentID, studentID, statusBook, description)
									VALUES(:appointmentID, :studentID, :statusBook, :description)");
			$stmt->execute(array(':appointmentID'=>$appID,':studentID'=>$sID, ':statusBook'=>$statusBook, 
								 ':description'=>$description));
			header('Location:approveAppointment.php');
			}
			catch(PDOException $ex){
				echo $ex->getMessage();
			}
		}
		else{ echo"INSERT INPUT";
		}
		}				
	?>
...
<body>
 ...
 <div class="container" style="width:49%">
  ...
  <h1> Status appointment </h1><br>
	
	<div class="radioButton">
    <label>Status</label><br>
	<input type="radio" name="statusBook" value="approve" class="radioButton"> Approve
	<input type="radio" name="statusBook" value="reject"class="radioButton"> Reject
    </div><br>
   ...
   <div class="form-group">
    ...
    <input type="submit" name="approve" id="approve" class="btn btn-info" value="Submit" />
...
/*$(document).ready(function(){
 ...
 $('#comment_form').on('submit', function(event){
  ...
  if($('#appointmentID').val() != '' && $('#studentID').val() != '' && $('#statusBook').val() != '' && $('#description').val() != '')
  {
